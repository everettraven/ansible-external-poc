...
install: ## Install ansible-operator
	go install $(GO_BUILD_ARGS) ./cmd/{ansible-operator,ansible-plugin}

.PHONY: build
build: ## Build ansible-operator
	@mkdir -p $(BUILD_DIR)
	go build $(GO_BUILD_ARGS) -o $(BUILD_DIR) ./cmd/{ansible-operator,ansible-plugin}

.PHONY:build/ansible-operator build/ansible-plugin
build/ansible-operator build/ansible-plugin:
...
.PHONY: image-build
IMAGE_TARGET_LIST = ansible-operator ansible-operator-2.11-preview
...
# Build an image.
BUILD_IMAGE_REPO = docker.io/bpalmer
...
.PHONY: test-unit
TEST_PKGS = $(shell go list ./... | grep -v -E 'github.com/everettraven/ansible-external-poc/test/')
...
export KIND_CLUSTER := ansible-test
